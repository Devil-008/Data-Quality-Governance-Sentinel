"""Databricks controller — uses connector credentials to query the Databricks REST API."""
import requests
...
@router.post("/clusters/{connector_id}")
def list_clusters(connector_id: int, user: dict = Depends(get_current_user)):
    base, headers = _ctx(connector_id)
    r = requests.get(f"{base}/api/2.0/clusters/list", headers=headers, timeout=20)
    if r.status_code != 200:
        raise HTTPException(status_code=400, detail=r.text[:300])
...
def list_runs(connector_id: int, user: dict = Depends(get_current_user)):
    base, headers = _ctx(connector_id)
    r = requests.get(f"{base}/api/2.1/jobs/runs/list?limit=25", headers=headers, timeout=20)
    if r.status_code != 200:
        raise HTTPException(status_code=400, detail=r.text[:300])
