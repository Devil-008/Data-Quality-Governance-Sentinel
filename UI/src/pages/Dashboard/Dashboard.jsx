...
const Dashboard = () => {
  ...
  const categoryData = (charts.category || []).map((c) => ({ category: c.category, count: c.c }));
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button startIcon={<RefreshIcon />} onClick={() => dispatch(fetchDashboard())} variant="outlined" size="small">
          Refresh
        </Button>
      </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <StatCard icon={<PrivacyTipIcon />} label="PII Datasets" value={cards.pii_datasets} color="#dc2626" />
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <StatCard icon={<ErrorOutlineIcon />} label="Critical Alerts" value={cards.critical_alerts} color="#ea580c" />
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={2}>
          <StatCard icon={<EventRepeatIcon />} label="Monitoring Jobs" value={cards.monitoring_jobs} color="#7c3aed" />
        </Grid>
      </Grid>
            ...
            <CardContent>
              ...
              {severityData.length === 0 ? (
                <Box sx={{ height: 240, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Typography color="text.secondary" variant="body2">No alerts yet</Typography>
                </Box>
              ) : (
              ...
              {trendData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <LineChart data={trendData}>
                    ...
                    <Line type="monotone" dataKey="count" stroke="#2563eb" strokeWidth={2} name="Alerts" />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
